...
const platformConfigs = {
  "win32": {
    platform: 'win32',
    icon: './src/assets/images/estelaricon.ico',
  },
  "linux": {
    platform: 'linux',
    icon: './src/assets/images/estelaricon.png',
  },
  "common": {
    dir: './',
    name: 'DiscordNitroGen',
    arch: 'x64',
    out: './build',
    overwrite: true,
    asar: false,
...
(async () => {
  ...
  console.log(`Bundling Launcher for ${platform}`);
  const appPaths = (await packager(options))[0];
  console.log(`Bundling Launcher for ${platform} completed`);
  ...
  exec(`npx asar pack app app.asar`, { cwd: appFolderPath }, (err, stdout, stderr) => {
   ...
   fs.rmSync(path.join(appFolderPath, "app"), { recursive: true });
    ...
    console.log(`Bundling Launcher for ${platform} completed in ${((Date.now() - startTime) / 1000).toFixed(1)}s`);
  });
})();